...
const navItems = [
  { to: "/", label: "Upload Report", icon: FileArrowUp, testId: "nav-link-upload" },
  { to: "/dashboard", label: "Dashboard", icon: ChartBar, testId: "nav-link-dashboard" },
...
export default function MainLayout() {
  return (
    <div className="report-app" data-testid="jira-compliance-dashboard">
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-[#E5E7EB] bg-white/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4 md:px-10">

          {/* Title */}
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-[#4B5563]">
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                  end={item.to === "/"}   // ✅ Fix for root active state
      ...
      {/* Page Content */}
      <main className="mx-auto min-h-[calc(100vh-90px)] w-full max-w-7xl px-6 py-10 md:px-10">
